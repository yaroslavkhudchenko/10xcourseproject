...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "cost today"
      decision: "before each purchase the shopper checks two or three shop sites by hand, every time"
    - topic: "pain category"
      decision: "data scattered across shops with no comparison"
    - topic: "insight"
      decision: "comparison sites skip these chains (Rossmann absent from Ceneo) and are too chaotic; a view focused on the exact product and the exact shops is missing"
    - topic: "primary persona scope"
      decision: "the owner first; a few people the owner knows as secondary users of the same deployment"
    - topic: "sign-in method"
      decision: "email and password"
    - topic: "sign-up"
      decision: "invite-only; no open registration"
    - topic: "roles"
      decision: "flat; every signed-in user has the same capabilities"
    - topic: "MVP scope and timeline"
      decision: "commit to the full seeded scope as the MVP; user estimate 2 to 3 weeks after-hours, recorded as 3; sustained-effort cost acknowledged 2026-09-19"
    - topic: "secondary success"
      decision: "owner buys where the app points; manual price entry becomes rare (user also marked some uncertainty)"
    - topic: "guardrails"
      decision: "stale or failed prices visible never silent; watchlists private; online prices labelled as online prices"
    - topic: "refresh cadence"
      decision: "on demand in the MVP (user opens a product or asks for a refresh); daily automatic refresh deferred as nice-to-have FR-015"
    - topic: "manual price entry"
      decision: "demoted to nice-to-have (FR-009); fetched prices are the only source in the MVP"
    - topic: "price per unit"
      decision: "dropped from the MVP comparison view; same-EAN comparison makes it redundant"
    - topic: "domain rule"
      decision: "cheapest of the shopper's shops now, plus a good-price judgement against the product's own history, or the shop's 30-day low until history exists; empty-CRUD check not triggered"
    - topic: "good-price inputs"
      decision: "product's own history; regular-vs-promo comparison not part of the rule; threshold and history length open"
    - topic: "day-one behaviour"
      decision: "judge against the shop's Omnibus 30-day low and label the comparison as such"
    - topic: "non-functional qualities"
      decision: "phone-usable at the shelf; continuous per-shop feedback while loading; every price shows its age; polite request cap per shop; private watchlists"
    - topic: "product type"
      decision: "web app"
    - topic: "target scale"
      decision: "a handful of users on one private deployment"
    - topic: "timing"
      decision: "hard deadline 2026-11-04 for the whole project; after-hours only"
    - topic: "scale probe"
      decision: "at 100x the rule stands but the data source would not; official feeds would be needed"
    - topic: "non-goals"
      decision: "permanent: no open sign-up, no shared household watchlists, no shelf or store-level prices; out of MVP but not permanent: perfumeries, alerts and reminders, manual entry, status page, daily refresh"
  frs_drafted: 15
...
> Personal price tracker for drugstore products bought repeatedly. A user searches a product by name and size, the app resolves it to an EAN and finds it in each supported shop through the shops' own search endpoints, documented in docs/research/polish-drugstore-price-apis.md. The user confirms the right item per shop once; afterwards the app refreshes prices daily by stored id or EAN, and the user can also record a price by hand. When buying, the user sees the cheapest shop, price per unit, regular and promo price with the Omnibus 30-day low, how fresh each price is, and whether it is a good price against that product's own history. Decided: multiple users with private watchlists over one shared price cache per EAN and shop; five shops (Rossmann, Hebe, Super-Pharm, dm, Drogerie Natura) added in stages; one adapter per shop with a health check; every price observation records its source; manual entry always available; tests run against recorded shop responses; no perfumeries and no alerts in the MVP. Open questions: which shops the user actually buys from and the order to add them; behaviour when a fetch fails; how much history and which threshold define a good price. Optional, not decided: run-out prediction with reminders, price alerts, Ceneo fallback for Douglas and Notino, store-level prices via Rossmann's store number, household sharing of a watchlist.
...
Comparison sites skip these chains: Rossmann is absent from Ceneo entirely and coverage of the others is patchy, and general comparison sites are too chaotic for this purpose. What is missing is a view focused on the exact product and the exact shops the shopper buys from. The prices themselves are public: each of the five chains exposes its current, promotional and 30-day-low price through its own product search (see docs/research/polish-drugstore-price-apis.md), so a focused comparison is feasible without any shop's cooperation.

Scale note (from the framing probe): at a hundred times the intended scale the rule itself would stand, but the shops' own search could not carry the traffic; official data feeds would be needed. The intended scale is a handful of people on one private deployment, with fetches made on demand and bounded by the per-shop request cap.
...
- (The owner marked some uncertainty about these; refine when the product is in use.)
...
- MVP scope committed: the full seeded scope (five shops added in stages, good-price rule, multi-user), with one change made in the Socrates round: refresh is on demand in the MVP and the daily automatic refresh is deferred (FR-015, nice-to-have).
- User estimate: 2 to 3 weeks of after-hours work; recorded as mvp_weeks: 3.
- Acknowledged on 2026-09-19: 3-week MVP requires sustained dedication; user accepted.
- Hard deadline: 2026-11-04. The owner applies it to the whole project as they define it, not only to the MVP flow. After-hours work only.
...
- Usable on a phone in the shop: the comparison is readable and operable on a current mobile browser while the shopper stands at the shelf. (Exact device and browser targets: refine in the PRD.)
...
- Sign-up: invite-only. The owner creates accounts or sends invites; there is no open registration.
- Roles: flat. Every signed-in user has the same capabilities: manage their own watchlist, confirm product matches, record manual prices, and see the shared price observations and adapter health.
- Data separation: watchlists are private per user; price observations are shared by all users of the deployment.
- Unauthenticated visitors: behaviour on a gated page not yet decided (see Open Questions).
...
- Manual price entry (FR-009), a separate adapter status page (FR-014) and the daily automatic refresh (FR-015) are nice-to-have and outside the MVP.
...
3. How are misspelled product names handled, and does search run live as you type or on submit? Rossmann's search returns a spelling hint; its suggestion endpoint is untested. — Owner: user.
...
Run on 2026-09-20 with status accepted. All checked elements present: Access Control, Business Logic (one-sentence rule), project artifacts, timeline-cost acknowledgment (mvp_weeks 3, acknowledged 2026-09-19), Non-Goals. Preserved-behavior check not applicable (greenfield). No gaps to mirror into the PRD beyond the six Open Questions above.

## Forward: tech-stack

Informational for the stack-selection step; not part of the PRD.

- Owner's stated preference from the pre-shaping conversation: TypeScript end to end. Candidate stack named by the owner: Astro, React, Tailwind, Supabase, Cloudflare. Python was considered for the backend and deferred to a possible later port of the shop adapters.
- Owner background: eight-plus years of front-end development, a year of agent-assisted delivery, no backend role so far. Learning goal for this project: backend fundamentals (SQL and data modelling, access policies, scheduled or on-demand jobs, deployment of something stateful).
- Data sources are the shops' own undocumented search endpoints, documented with probes and sample responses in docs/research/polish-drugstore-price-apis.md. Two shops need dynamic values read from their pages at runtime (Super-Pharm search key, Luigi's Box tracker ids). Egress from the chosen hosting should be verified against the five shops before committing.

## Forward: technical-roadmap

Informational for implementation planning; not part of the PRD.

- One adapter per shop behind a common interface, each with a health check on a known EAN; a response-shape validation failure counts as an unhealthy adapter.
- Tests run against recorded shop responses, never live shops; a live health check may exist outside the gating suite.
- Adapters staged: dm, Drogerie Natura and Rossmann first (clean EAN paths), then Hebe and Super-Pharm.
- Every price observation records its source and timestamp; observations are shared across users, watchlists are private.
- A demo account seeded from recorded data so the app can be shown working without generating traffic to the shops.
- Deferred items the owner listed as optional, not decided: run-out prediction with reminders, price alerts, Ceneo fallback for Douglas and Notino, store-level prices via Rossmann's store number, household sharing of a watchlist, daily automatic refresh (FR-015).
