...
- **One `### Phase N: <name>`** per phase, in order, matching the descriptive `## Phase N:` or `## Faza N:` headers earlier in the plan **by phase number**. The descriptive heading language is not a consistency defect; keep Polish `Faza` headings. Progress retains its canonical `### Phase N:` headings.
...
- `/10x-archive` may replace an existing SHA suffix on explicitly approved `[x]` rows with a verified integration commit before moving the folder, after showing the old → new mapping and obtaining confirmation. It records the mapping, row IDs, target branch and evidence in `reviews/archive-sha-repoint.md` and includes both files in the archive commit. This exception never changes checkboxes, indices, titles, pending rows or SHA-less rows. Without approval, Progress stays unchanged; existing archives are never rewritten.
